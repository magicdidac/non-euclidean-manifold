...
public class MinePortal : MonoBehaviour
{
    ...
    public MinePortal linkedPortal;
    public MinePortal[] alsoVisiblePortals;
    ...
    protected RenderTexture viewTexture;
    protected Camera portalCam;
    ...
    {
        if(!viewTexture || viewTexture.width != Screen.width || viewTexture.height != Screen.height)
        {
            if (viewTexture)
            {
                viewTexture.Release();
    ...
    private void OnDrawGizmos()
    {
        ...
        if (transform.name.CompareTo(linkedPortal.name) < linkedPortal.name.CompareTo(transform.name))
            return;

        Gizmos.color = Color.green;

        Gizmos.DrawSphere(transform.position + (transform.forward * 1.5f), .25f);
        Gizmos.DrawSphere(linkedPortal.transform.position + (-linkedPortal.transform.forward * 1.5f), .25f);
